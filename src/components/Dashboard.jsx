...
export default function Dashboard({ isDark, date, zone, setZones, selectedPlants }) {
  ...
  (selectedPlants || []).map(String)
);

       opsData.forEach(({ plantId, operation }) => {
  if (!operation) return;

  // When specific plants are selected, only process those plants.
  // When no plants are selected, process all plants.
  if (
    selectedPlants?.length > 0 &&
    !selectedPlants.map(String).includes(String(plantId))
  ) {
    return;
  }

  if (operation.pilletsStock != null) {
    pelletsStock += Number(operation.pilletsStock);

    if (
      operation.pilletsStock === 0 ||
      operation.pilletsStock <= 100
    ) {
  ...
  if (operation.polymerStock != null) {
    polymerStock += Number(operation.polymerStock);

    if (
      operation.polymerStock === 0 ||
      operation.polymerStock <= 5
    ) {
